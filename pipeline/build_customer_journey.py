...
class CustomerJourneyBuilder:
        ...
        try:
            with self.get_connection() as conn:
                ...
                if journeys_df.empty:
                    logger.warning(
                        "No customer journeys found in the database. Exiting."
                    )
                    exit(0)
                ...
                logger.info(
                    f"Built {len(journeys_df)} journey records for {journeys_df['conv_id'].nunique()} conversions"
                )
    ...
    def get_journey_stats(self, df: pd.DataFrame) -> Dict:
        ...
        stats = {
            ...
            "avg_revenue_per_conversion": df.groupby("conv_id")["revenue"]
            .first()
            .mean(),
        }
